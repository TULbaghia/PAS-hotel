package pl.lodz.p.pas.service.filters.fiveStarFilters.updateFiveApartmentFilters;
...
public class PutFiveApartmentCheck implements ContainerRequestFilter {
    @Override
    public void filter(ContainerRequestContext requestContext) {
        ...
        try {
            if (jsonObject.getString("pcName").isEmpty()) {
                errorPropList.add(new ErrorProp("pcName", "is not defined"));
            }
        } catch (JSONException e) {
            errorPropList.add(new ErrorProp("", e.getMessage()));
        }
